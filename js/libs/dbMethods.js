 ...
 */
function checkDB(callback2,params) {
	if ( typeof (db) != 'undefined') {
		...
		function start() {
			var callback = function(str) {
				...
				db = new SQL.Database(uInt8Array);
				console.log(db);
				console.log("database nearly loaded");
				callback2(params);
			};
...
/**
 *  Load the database named './wavelet_database.sqlite' (with calling a function setHtmlFunctions(),
 *  such that no additional waiting-tricks, like a timer, are needed).
 *  (must exist in the same folder as the html-file) in a new thread.
 *  (last modification: 26.1.17 Andreas)
 */
function loadWaveletDB2() {
	//builds an HttpRequest on 'theUrl' and runs the 'callback' function with the content of the HttpRequest response
	function httpGetAsync(theUrl, callback) {
		var xmlHttp = new XMLHttpRequest();
		xmlHttp.responseType = 'arraybuffer';
	...
	function start() {
		...
		var callback = function(str) {
			...
			//When the database is loaded, call the function "setHtmlFunctions()".
			setHtmlFunctions()
