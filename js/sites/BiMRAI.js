...
function setHtmlFunctions() {
	...
	document.getElementById('select-derivative-order').onchange = function() {
		...
		var scf = db.exec("SELECT * FROM BiMRA-Wavelets WHERE id=" + idOfScf)[0].values[0];
...
function updateScfList() {
	...
	var newstr = "SELECT * FROM BiMRA-Wavelets WHERE " + str;
...
function updateDualScfList(id) {
	cleanWaveletPlot();
	var command1 = "SELECT ID_of_dual_function FROM BiMRA-Wavelets WHERE ID ="
			+ id;
	var str = db.exec(command1)[0].values[0][0];

	if (str != undefined) {
		var noAr = stringToNoArray(str);
		var command2 = "SELECT * FROM BiMRA-Wavelets WHERE";
...
function getCoeffs(idOfScf) {
	var coeffsAsString = db.exec("SELECT mask FROM BiMRA-Wavelets "
			+ "WHERE id=" + idOfScf)[0].values[0][0];
	var a = stringToNoArray(coeffsAsString);
	a_start = db.exec("SELECT a_start FROM BiMRA-Wavelets " + "WHERE id="
