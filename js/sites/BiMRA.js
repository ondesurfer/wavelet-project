...
function setHtmlFunctions() {
	...
	document.getElementById('select-derivative-order').onchange = function() {
		...
		var scf = db.exec("SELECT * FROM BiMRA WHERE id=" + idOfScf)[0].values[0];
...
function updateDualScfList(id) {
	cleanWaveletPlot();
	var command1 = "SELECT ID_of_dual_function FROM BiMRA WHERE ID ="
			+ id;
	var str = db.exec(command1)[0].values[0][0];

	if (str != undefined) {
		var noAr = stringToNoArray(str);
		var command2 = "SELECT * FROM BiMRA WHERE";
...
function getCoeffs(idOfScf) {
	var coeffsAsString = db.exec("SELECT mask FROM BiMRA "
			+ "WHERE id=" + idOfScf)[0].values[0][0];
	var a = stringToNoArray(coeffsAsString);
	a_start = db.exec("SELECT a_start FROM BiMRA " + "WHERE id="
