/**
 * this file contains the method to add the functions to some html elements on the Fwavelet-plot.html file
 */
	...
	function setHtmlFunctions(){
		...
		document.getElementById('input8').onclick = function() {
				//Fehlerabfrage noetig!!
				var c_t=getCoeffs(document.getElementById('select3').value);
				var d_t=getCoeffs(document.getElementById('select4').value);
				//calculates the new derivative values and saves it globally
 ...
 */			
		function cleanPlotAndInfo(){
			...
			plotInstance2.draw();
			cleanWaveletPlot();
			document.getElementById('select4').length=0;
		}
 ...
 */			
		function updateScfList(){
			...
			fillList(document.getElementById('select3'),currentdb);
		}
/** updates the List containing the dual scalingfunctions fitting to the choosen scf
 *  (last modification: 26.7.16 Simon)
 * @param id ID of the main scaling function
 */	
		function updateDualScfList(id){
			cleanWaveletPlot();
			var command1 = "SELECT ID_of_dual_function FROM ScalingFunctionsSupp WHERE ID ="+id;
			...
			if(str!= undefined){
				...
				fillList(document.getElementById('select4'),currentdb);
			}
...
/** fills a selectBox with the scalingfunctions in the new database
 *  (last modification: 26.7.16 Simon)
 * 
 * @param {list} selectBox which should be filled
